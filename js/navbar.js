...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(nav && !nav.querySelector('a[href="news.html"]')){
    const newsLink=document.createElement('a'); newsLink.href='news.html';
    const syncNewsLabel=()=>{newsLink.textContent=(document.documentElement.lang||'vi').toLowerCase().startsWith('en')?'Technology News':'Tin tức công nghệ';};
    syncNewsLabel();
    if(location.pathname.endsWith('/news.html') || location.pathname.endsWith('news.html')) newsLink.classList.add('active');
    const statsExisting=nav.querySelector('a[href="stats.html"]');
    const languageToggle=nav.querySelector('[data-language-toggle]');
    if(statsExisting) nav.insertBefore(newsLink,statsExisting);
    else if(languageToggle) nav.insertBefore(newsLink,languageToggle);
    else nav.appendChild(newsLink);
    if('MutationObserver' in window)new MutationObserver(syncNewsLabel).observe(document.documentElement,{attributes:true,attributeFilter:['lang']});
  }
  ...
  const motionCards=[...document.querySelectorAll('.expertise-card,.product-card,.research-card,.stat-card,.table-card,.trend-card,.donut-card,.news-card')];motionCards.forEach(card=>card.classList.add('motion-card'));
