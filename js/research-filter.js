document.addEventListener('DOMContentLoaded',()=>{
  ...
  const paperHeading=document.querySelector('.research-section-heading');
  ...
  const apply=()=>{
    ...
    cards.forEach(card=>{
      ...
      card.hidden=!ok;
      if(ok){visible++;if(card.classList.contains('paper-card'))visiblePapers++;}
    });
    if(count)count.textContent=String(visible);
    if(empty)empty.hidden=visible!==0;
    if(paperHeading)paperHeading.hidden=visiblePapers===0;
  };
