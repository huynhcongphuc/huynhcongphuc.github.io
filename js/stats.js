document.addEventListener('DOMContentLoaded',()=>{
  ...
  const TEXT={vi:{noData:'Chưa có dữ liệu',updated:'Cập nhật lúc',unknown:'Không xác định',vietnam:'Việt Nam',users:'Người dùng',views:'Lượt xem',software:'Tải phần mềm',research:'Tải nghiên cứu'},en:{noData:'No data',updated:'Updated at',unknown:'Unknown',vietnam:'Vietnam',users:'Users',views:'Views',software:'Software downloads',research:'Research downloads'}};
  ...
  function renderUnifiedChart(){
    ...
    const totals={
      users:Number(allPeriod.users||0),
      views:Number(allPeriod.views||rows.reduce((s,r)=>s+Number(r.views||0),0)),
      software_downloads:Number(allPeriod.downloads||rows.reduce((s,r)=>s+Number(r.software_downloads||0),0)),
      research_downloads:Math.max(Number(allPeriod.research_downloads||0),researchAll.reduce((s,r)=>s+Number(r.downloads||0),0))
    };
  ...
  function render(){
    ...
    const researchRows=document.getElementById('research-rows');if(researchRows)researchRows.innerHTML=researchData.map(x=>`<tr><td>${esc(x.name)}</td><td>${fmt.format(x.downloads||0)}</td></tr>`).join('');renderUnifiedChart();
    const updated=document.getElementById('updated-at');if(updated&&stats.updated_at)updated.textContent=`${TEXT[l].updated} ${new Date(stats.updated_at).toLocaleString(l==='en'?'en-US':'vi-VN')}`;const error=document.getElementById('stats-error');if(error)error.style.display='none';
  }
  ...
  new MutationObserver(m=>{if(m.some(x=>x.type==='attributes'&&x.attributeName==='lang'))setTimeout(render,0);}).observe(document.documentElement,{attributes:true});loadStats();setInterval(loadStats,60000);document.addEventListener('visibilitychange',()=>{if(!document.hidden)loadStats();});
});